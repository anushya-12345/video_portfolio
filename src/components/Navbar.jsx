      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="h-20 flex items-center justify-between">
          ...
          <button
            ...
            className="hidden md:block px-5 py-2.5 rounded-full bg-white text-black text-sm font-bold hover:bg-gray-200 transition-all"
          >
            Let's Connect
          </button>
          ...
          <div className="px-6 py-5 flex flex-col gap-4">
            ...
            {navLinks.map((link) => (
              <button
                key={link.id}
                onClick={() => handleNavigation(link.id)}
                className="text-left text-white font-semibold py-2"
              >
                {link.name}
              </button>
            ))}
